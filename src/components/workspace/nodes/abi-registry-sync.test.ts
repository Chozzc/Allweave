...
import { NODE_TYPE_TO_ABI_FEATURE } from "tongflow";
import { describe, expect, it } from "vitest";

const WORKSPACE_DIR = join(process.cwd(), "src", "components", "workspace");
...
/** nodeType → component file, from the `nodeTypes` map in types.tsx. */
function nodeTypeFiles(): Record<string, string> {
    const src = readFileSync(join(WORKSPACE_DIR, "types.tsx"), "utf8");
    const imports = new Map<string, string>();
    for (const m of src.matchAll(/import (\w+) from "\.\/(nodes\/[^"]+)";/g)) {
        imports.set(m[1], join(WORKSPACE_DIR, `${m[2]}.tsx`));
    }
...
describe("ABI node components ↔ static registry", () => {
    ...
    it("no component carries its own sourceSpec (registry is the truth)", () => {
        for (const file of walk(join(WORKSPACE_DIR, "nodes"))) {

...
Copy one with `tongflow_workflow_new({ path, fromTemplate: 'workflows/<name>.tongflow.json' })`, then bind its inputs (`tongflow_workflow_bind`) and run. Plugins are filled with the installed default for each slot when copied.

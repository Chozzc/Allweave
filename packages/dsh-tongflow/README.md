...
- The Studio tree nests a workflow's outputs under it; the user may rename / move / delete anything by hand — the agent re-reads the tree (`tongflow_project_status`) before acting.

### Billing checkpoint

A run spends the user's money — a paid API key, or GPU seconds on their Modal account (a Modal plugin also deploys on first use). So the first time a plugin (or a specific model) is used in a project, `tongflow_workflow_run` does not run: it returns `needs_confirmation` with the plugins involved, how each is billed, whether its API keys are set, the models it offers and installed alternatives. The agent puts that to the user, and only after an explicit yes records the choice with `tongflow_plugin_approve` (kept in `project.json` → `plugins`) and runs. The Studio's own **Run** drawer shows the same notice and a **Confirm & run** button. Approval is per project; revoke it to be asked again.

## Agent tools

`tongflow_project_create / _open / _list / _status` · `tongflow_workflow_new / _patch / _read / _list / _validate / _run` · `tongflow_plugin_approve` · `tongflow_node_catalog / _describe` · `tongflow_look` (images / video contact sheets, returned as an image block) · `tongflow_perceive` (video/audio/image understanding via TongFlow slots) · `tongflow_plugins_list / _install / _uninstall` · `tongflow_run_status`. Folder structure and text files are made with dsh's ordinary file tools. Long runs go through dsh background jobs (`run_in_background`).

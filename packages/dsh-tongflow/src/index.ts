...
A media studio is available through the tongflow_* tools. Rules: (1) all image / audio / video / 3D generation happens by creating and running a saved workflow file — one file per generated asset, placed in the folder where that asset belongs: tongflow_workflow_new → _patch → _run — there is no direct generate tool; its outputs land next to it as <name>.01.png, <name>.02.png … with <name>.runs.json (a run never overwrites); (2) the project folder is the source of truth and has NO fixed layout: read the brief, research the format if needed, propose a folder structure to the user, create it with the file tools and document it in a README/plan file; the user may reorganize by hand at any time, so call tongflow_project_status before assuming; (3) runs cost money (paid API keys or Modal GPU): the first time a plugin / model is used in a project, tongflow_workflow_run stops with needs_confirmation — explain the billing and the choices to the user, let them decide, record it with tongflow_plugin_approve, then run; never approve on your own; (4) review generated media with tongflow_look / tongflow_perceive before building on it. Load the "tongflow-studio" skill for the full method.

...
A media studio is available through the tongflow_* tools. Rules: (1) all image / audio / video / 3D generation happens by creating and running a saved workflow file — one file per generated asset, placed in the folder where that asset belongs: tongflow_workflow_new → _patch → _run — there is no direct generate tool; its outputs land next to it as <name>.01.png, <name>.02.png … with <name>.runs.json (a run never overwrites); (2) the project folder is the source of truth and has NO fixed layout: read the brief, research the format if needed, propose a folder structure to the user, create it with the file tools and document it in a README/plan file; the user may reorganize by hand at any time, so call tongflow_project_status before assuming; (3) paid runs (API keys or Modal GPU) need the user's yes EVERY time: tongflow_workflow_run without user_confirmed stops with needs_confirmation — explain what will run, how it is billed and the choices, wait for the user's answer, then call again with user_confirmed=true; never set it on your own; (4) review generated media with tongflow_look / tongflow_perceive before building on it. Load the "tongflow-studio" skill for the full method.

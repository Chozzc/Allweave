...
- **Billing checkpoint**: `project.json.plugins` lists the plugins (optionally models) the user approved. `StudioApi.unapprovedPlugins(project, workflow)` diffs the workflow's executable nodes against it, classifying each plugin as `modal` (registry `needsDeploy`), `api` (a required env key) or `local`; the agent run tool and the Studio run drawer both stop on a non-empty answer and record consent via `approvePlugin`. Canvas single-node runs are user-initiated with the plugin/model visible on the node and are not gated.

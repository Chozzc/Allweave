...
在线版本无需注册，打开后可以直接查看示例画布、编辑节点，并在当前浏览器保存工作流、任务历史、生成素材、作品集和个人配置。

## 画布与工作流预览

| 基础图像工作流 | 生成结果 |
| --- | --- |
| <img src="https://file.tongflow.com/public/demos/basic.png" alt="基础图像生成与融合工作流" width="720" /> | <img src="https://file.tongflow.com/public/demos/basic_result.png" alt="基础工作流生成结果" width="240" /> |

### 数字人口播工作流

<p align="center">
  <img src="https://file.tongflow.com/public/demos/digitalhuman.png" alt="数字人口播工作流" width="920" />
</p>

### 音乐视觉与 MV 工作流

<p align="center">
  <img src="https://file.tongflow.com/public/demos/mv.png" alt="音乐视觉与 MV 工作流" width="920" />
</p>

> 上述流程图来自上游 [TongFlow](https://github.com/tong-io/tongflow) 官方示例，用于展示 Allweave 保留并继续扩展的多模态画布能力。Allweave 的比赛模板、品牌和浏览器本地存储是在此基础上的二次开发。
...
- **任务与作品管理**：任务历史、生成结果、收藏状态和作品集均持久化到当前浏览器的 IndexedDB。
- **双语界面**：工作区支持中英文切换。
...
- 本地开发目录已经安装完整的 47 个官方插件；插件包括声明文件、节点实现和 Python 执行代码。
- 当前仓库沿用了上游的 `/plugins/*` 忽略规则，因此这批运行时代码尚未随 GitHub 仓库和 Vercel 构建发布；网站仍会显示安装入口。
- Allweave 的目标形态是把官方插件作为随站点发布的内置能力，用户无需逐个安装。API Key 和个人选择仍由每位用户在浏览器中自行配置。
- Modal、OpenRouter 等只是插件所连接的不同模型或算力服务，不等同于插件本身，也不是打开、编辑和保存画布的前置条件。

## 数据与隐私

- 不需要注册或登录。
- 工作流、画布草稿、任务历史、生成素材、作品集状态和用户配置均保存在当前设备的 IndexedDB 中，不会自动同步到其他浏览器。
- API Key 采用 BYOK（自带密钥）模式，不会提交到 Git 仓库。
- 清除浏览器站点数据会删除本地草稿，重要工作流应先导出 JSON。
- 服务端的临时任务数据库只承担执行期间的兼容与中转，不是用户长期数据源；持久记录由浏览器 IndexedDB 负责。
...
├── React 19 + Next.js 15
├── React Flow 无限画布
├── IndexedDB 工作流、任务、素材、作品集与 BYOK 设置
└── 工作流导入 / 导出
        │
        ▼
Workflow Core
...
- 浏览器 IndexedDB 工作流、任务历史、素材、作品集与 BYOK 设置。
...
1. 将 47 个官方插件作为网站内置资源发布，取消用户逐个安装的步骤。
2. 为三条示例工作流补齐可稳定运行的默认节点、错误提示和演示素材。
3. 增加首页、模板入口和自然语言工作流助手。
4. 增加工作流分享、导入预览和模板市场能力。
5. 完成移动端适配、性能优化、异常恢复和端到端测试。
...
工作流、任务历史、生成素材、作品集和浏览器设置均保存在当前浏览器的 IndexedDB。服务器端临时数据仅用于执行兼容与中转。
